      ...
      await Promise.all(
        symbolsToFetch.map(async symbol => {
          try {
            ...
            companyNames[symbol] = profileData.name || symbol;
          } catch {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (dayData.preMarket.length > 0) {
          ...
          dayData.preMarket.forEach((earning, idx) => {
            const companyName = companyNames[earning.symbol] || earning.symbol;
            ...
            html += `<div style="color: var(--text-secondary); font-size: 0.875rem;">EPS Est: ${earning.epsEstimate || 'N/A'}</div>`;
            html += '</div>';
          });
          html += '</div>';
        } else {
...
async function renderEarningsCalendar() {
  ...
  try {
    ...
    const response = await fetch(`https://finnhub.io/api/v1/calendar/earnings?from=${today}&to=${today}&token=${API_KEY}`);
    const data = await response.json();
    ...
    const usEarnings = data.earningsCalendar.filter(earning => {
      const symbol = earning.symbol || '';
      // US tickers are typically 1-5 uppercase letters without dots or special characters
      return /^[A-Z]{1,5}$/.test(symbol) && earning.hour && (earning.hour === 'bmo' || earning.hour === 'amc');
    });
