...
function renderMarketIntelligence() {
  // Economic Calendar - FIXED: Use correct script embed
  const economicContainer = document.getElementById('tv-economic-calendar');
  if (economicContainer) {
    economicContainer.innerHTML = ''; // Clear previous
  ...
  // Earnings Calendar - FIXED: Use correct earnings widget
  const earningsContainer = document.getElementById('tv-earnings-calendar');
  if (earningsContainer) {
    earningsContainer.innerHTML = ''; // Clear previous
    const earningsScript = document.createElement('script');
    earningsScript.type = 'text/javascript';
    earningsScript.src = 'https://s3.tradingview.com/external-embedding/embed-widget-earnings.js';
    earningsScript.async = true;
    earningsScript.innerHTML = JSON.stringify({
      colorTheme: 'light',
      isTransparent: false,
      width: '100%',
      height: '400',
    ...
    earningsContainer.appendChild(earningsScript);
  }

  // Market News - Already correct
