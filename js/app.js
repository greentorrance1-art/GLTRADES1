...
async function importTradesFromCSV(csvText) {
  try {
    ...
    const getColIndex = (name) => header.findIndex(h => h.toLowerCase().includes(name.toLowerCase()));

    const symbolIdx = getColIndex('symbol');
    const qtyIdx = getColIndex('qty');
    const buyPriceIdx = getColIndex('buy price');
    const buyTimeIdx = getColIndex('buy time');
    const sellPriceIdx = getColIndex('sell price');
    const sellTimeIdx = getColIndex('sell time');
    const plIdx = getColIndex('p&l') >= 0 ? getColIndex('p&l') : getColIndex('pl');

    if (symbolIdx === -1 || qtyIdx === -1 || buyPriceIdx === -1 || sellPriceIdx === -1) {
      alert('CSV file is missing required columns (Symbol, Qty, Buy Price, Sell Price).');
      return;
    }
    ...
    // Process each row asynchronously to avoid UI freezing
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        const entryPrice = parseFloat(row[buyPriceIdx]);
        const exitPrice = parseFloat(row[sellPriceIdx]);
        const pl = plIdx >= 0 ? parseFloat(row[plIdx]) : (exitPrice - entryPrice) * qty;

        // Parse times (format: "01/07/2026 10:52:30")
        ...
        const entryTime = buyTimeIdx >= 0 ? parseTime(row[buyTimeIdx]) : null;
        const exitTime = sellTimeIdx >= 0 ? parseTime(row[sellTimeIdx]) : null;

        // Extract date from entry time for the trade date field
        ...
        // Check for partial duplicate (same date and entry time)
        const isPartialDuplicate = window.trades.some(t =>
          t.date === tradeDate &&
          t.entryTime === entryTime
        );

        if (isPartialDuplicate) {
          console.log(`Skipping potential duplicate: ${symbol} on ${tradeDate} at ${entryTime}`);
          skipped++;
          continue;
        }

        // Determine side based on P&L (positive = long, negative = short assumed)
        const side = pl >= 0 ? 'long' : 'short';
        ...
        // Create trade object matching platform structure
        const tradeData = {
          date: tradeDate,
        ...
        console.error(`Error processing row ${i + 1}:`, err, row);
        errors++;
      }
    }
